...
exports.createPrice = (req, res) => {
  ...
  connection.query(
    "INSERT INTO price_cat (pname, pbreakfast, plunch, pdinner) VALUES (?, ?, ?, ?)",
    [pname, pbreakfast, plunch, pdinner],
    (err, rows) => {
      if (!err) {
        connection.query("SELECT * FROM price_cat", (errs, rows) => {
          if (!errs) {
            res.render("price", {
              title: "Pricing",
              success: "Price Category added successfully.",
              rows,
            });
          }
        });
...
exports.editPriceView = (req, res) => {
  ...
  connection.query(
    "SELECT * FROM price_cat where pid = ?",
    [pid],
    (err, rows) => {
      if (!err) {
...
exports.editPrice = (req, res) => {
  ...
  connection.query(
    "UPDATE price_cat SET pname = ? , pbreakfast = ? , plunch = ? , pdinner = ? WHERE pid = ?",
    [pname, pbreakfast, plunch, pdinner, pid],
    (err, rows) => {
      if (!err) {
        connection.query("SELECT * FROM price_cat", (errs, rows) => {
          if (!errs) {
            res.render("price", {
              title: "Pricing",
              success: "Price Category updated successfully.",
              rows,
            });
          }
        });
...
exports.home = (req, res) => {
  ...
  connection.query(
    "SELECT * FROM cust, price_cat WHERE cust.pid = price_cat.pid AND cname LIKE ? ORDER BY grp",
    ["%" + name + "%"],
    (err, rows) => {
      if (!err) {
        connection.query(
...
exports.homeToggle = (req, res) => {
  ...
  if (m === "a") {
    connection.query(
      "SELECT * FROM meal WHERE cid = ? and date = ?",
      [id, d],
      (err, rows) => {
        if (rows.length) {
          const sqlString = `UPDATE meal SET b = ?, l = ?, d = ? WHERE cid = ? and date = ?`;
          connection.query(sqlString, [o, o, o, id, d], (err1, rows1) => {
            if (!err1) {
              this.home(req, res);
            } else {
              res.render("alertpage", { title: "Error", error: err1 });
          ...
          const sqlString = `INSERT INTO meal (cid, date, b, l, d) VALUES (?, ?, ?, ? , ?)`;
          connection.query(sqlString, [id, d, o, o, o], (err1, rows1) => {
            if (!err1) {
              this.home(req, res);
            } else {
              res.render("alertpage", { title: "Error", error: err1 });
...
exports.rangeToggle = (req, res, next) => {
  const cid = req.params.id;
  const { sdate, edate, b, l, d } = req.body;
  let sqlInsert = "INSERT INTO meal (cid, date, b, l, d)  VALUES";
  let sqlDelete = `DELETE FROM meal WHERE cid = ${cid} AND date IN (`;
  for (var m = moment(sdate); m.diff(edate, "days") <= 0; m.add(1, "days")) {
    sqlInsert += `(${cid}, "${m.format("YYYY-MM-DD")}", ${b}, ${l}, ${d})`;
    sqlDelete += `"${m.format("YYYY-MM-DD")}"`;
    if (m.diff(edate, "days") != 0) {
      sqlInsert += ",";
      sqlDelete += ",";
    } else sqlDelete += ")";
  }
  connection.query(sqlDelete, (err, rows) => {
    if (!err) {
      connection.query(sqlInsert, (err1, rows1) => {
        if (!err1) {
          next();
        } else {
          res.render("alertpage", { title: "Error", error: err });
...
exports.allToggle = (req, res) => {
  ...
  connection.query(
    sqlInsert,
    [req.params.d, req.params.o, req.params.d],
    (err, rows) => {
      if (!err) {
        connection.query(
          sqlUpdate,
          [req.params.o, req.params.d],
          (err1, rows) => {
            if (!err1) {
              this.home(req, res);
            } else {
...
exports.createCustomer = (req, res) => {
  ...
  connection.query(
    "INSERT INTO cust (cname, mstart, mend, grp, pid) VALUES (?, ?, ?, ?, ?)",
    [cname, mstart, mend, grp, pid],
    (err, rows) => {
      if (!err) {
        connection.query("SELECT * FROM price_cat", (errs, rows) => {
          if (!errs) {
      ...
      connection.query("SELECT * FROM price_cat", (err1, rows1) => {
        if (!err1) {
          ...
          rows1.forEach((element) => {
            if (element.pid === rows[0].pid) element.selected = 1;
          });
          res.render("editCustomer", { title: "Edit Customer", row, rows1 });
        } else res.render("alertpage", { title: "Error", error: err });
      });
    } else {
      res.render("alertpage", { title: "Error", error: err });
    }
...
exports.editCustomer = (req, res) => {
  ...
  connection.query(
    "UPDATE cust SET cname = ?, pid = ?, grp = ? WHERE cid = ?",
    [cname, pid, grp, cid],
    (err, rows) => {
      if (!err) {
...
exports.bill = (req, res) => {
  ...
  connection.query(
    "SELECT * FROM bill WHERE cname LIKE ? ORDER BY mend DESC",
    ["%" + name + "%"],
    (err, rows) => {
      if (!err) {
        rows.forEach((element) => {
...
function billList(date) {
  connection.query(
    "SELECT * FROM cust WHERE mend <= ?",
    [date],
    (err, rows) => {
      for (i = 0; i < rows.length; i++) {
        connection.query(
          "INSERT INTO bill (cid, cname, mstart, mend, pid) VALUES (?, ?, ?, ?, ?)",
          [
            rows[i].cid,
            rows[i].cname,
            moment(rows[i].mstart).format("YYYY-MM-DD"),
            moment(rows[i].mend).format("YYYY-MM-DD"),
            rows[i].pid,
          ]
        );
        connection.query("UPDATE cust SET mstart = ?, mend = ? WHERE cid = ?", [
          moment(rows[i].mend).add(+1, "days").format("YYYY-MM-DD"),
          moment(rows[i].mend).add(+30, "days").format("YYYY-MM-DD"),
          rows[i].cid,
        ]);
      }
    }
    ...
    (err, rows) => {
      if (!err) {
        if (Number(moment().diff(moment(rows[0].mstart)), "days") < 0) {
          res.render("alertpage", {
            title: "INFO",
            error: `Bill for ${rows[0].cname} can not be generated today.`,
          });
        } else {
          connection.query(
            "INSERT INTO bill (cid, cname, mstart, mend, pid) VALUES (?, ?, ?, ?, ?)",
            [
              rows[0].cid,
              rows[0].cname,
              moment(rows[0].mstart).format("YYYY-MM-DD"),
            ...
            (err1, rows1) => {
              if (!err1) {
                connection.query(
                  "UPDATE cust SET mstart = ?, mend = ? WHERE cid = ?",
                  [
                  ...
                  (err2, rows2) => {
                    if (!err2) {
                      res.render("alertpage", {
                        title: "Bill",
                        success: `Bill for ${rows[0].cname} generated successfully.`,
                      });
                    } else {
...
exports.genrateBill = (req, res) => {
  ...
  var frows = [];
  connection.query(
    "SELECT * FROM bill WHERE bid =?",
    [req.params.id],
    (berr, brows) => {
      if (!berr) {
        connection.query(
          "SELECT * FROM meal WHERE date BETWEEN ? AND ? AND cid = ? ORDER BY date",
          [
            moment(brows[0].mstart).format("YYYY-MM-DD"),
            moment(brows[0].mend).format("YYYY-MM-DD"),
            brows[0].cid,
          ],
          (err, rows) => {
            if (!err) {
              connection.query(
                ...
                (perr, prows) => {
                  if (!perr) {
                    const period =
                      moment(brows[0].mend).diff(
                        moment(brows[0].mstart),
                        "days"
                      ) + 1;
                    for (i = 0; i < period; i++) {
                      frows.push({
                        date: moment(brows[0].mstart)
                          .add(i, "days")
                          .format("DD-MM-YYYY"),
                        b: 1,
                        l: 1,
                    ...
                    total =
                      prows[0].pbreakfast * period +
                      prows[0].plunch * period +
                      prows[0].pdinner * period;
                    rows.forEach((element) => {
                      element.date = moment(element.date).format("DD-MM-YYYY");
                      if (element.b == 0) total = total - prows[0].pbreakfast;
                      if (element.l == 0) total = total - prows[0].plunch;
                      if (element.d == 0) total = total - prows[0].pdinner;
                    });
                    ...
                    res.render("billdetails", {
                      title: "Bill",
                      cat: prows[0].pname,
                      pbreakfast: prows[0].pbreakfast,
                      plunch: prows[0].plunch,
                      pdinner: prows[0].pdinner,
                      bid: brows[0].bid,
                      name: brows[0].cname,
                      mstart: moment(brows[0].mstart).format("DD-MM-YYYY"),
                      mend: moment(brows[0].mend).format("DD-MM-YYYY"),
                      frows,
                      total,
                    });
                  } else {
...
exports.deleteBill = (req, res) => {
  connection.query(
    "DELETE FROM meal WHERE date >= (SELECT mstart FROM bill WHERE bid = ?) AND date <= (SELECT mend FROM bill WHERE bid = ?) AND cid = (SELECT cid FROM bill WHERE bid = ?)",
...
exports.deleteMultiBill = (req, res, next) => {
  const { billIds } = req.body;
  connection.query(
    "DELETE FROM bill WHERE bid IN (?)",
    [billIds],
    (err, rows) => {
      if (!err) {
        next();
